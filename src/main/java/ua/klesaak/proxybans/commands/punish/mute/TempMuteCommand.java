package ua.klesaak.proxybans.commands.punish.mute;
...
public final class TempMuteCommand extends AbstractPunishCommand {

    public TempMuteCommand(ProxyBansManager proxyBansManager) {
        super(proxyBansManager, PunishType.TEMP_MUTE);
    }
    ...
    public boolean onReceiveCommand(CommandSender sender, String[] args) throws AbstractCommandException {
        ...
        this.cmdVerifyArgs(4, args, messagesFile.getUsageTempMuteCommand());
        ...
        messagesFile.getBroadcastTempMuted()
        ...
        messagesFile.getMessageTempMuted()
                .tag(PUNISHER_NAME_PATTERN, punisherName)
                ...
                .tag(PLAYER_SERVER_PATTERN, playerServer)
                .tag(TIME_PATTERN, punishExpireDate)
                .tag(DATE_PATTERN, date).send(nickName);

        val punishData = PunishData.builder()
                .playerName(nickName)
                .punishType(PunishType.TEMP_MUTE)
        ...
        this.proxyBansManager.getPunishStorage().saveMute(punishData);
        return true;
    }
